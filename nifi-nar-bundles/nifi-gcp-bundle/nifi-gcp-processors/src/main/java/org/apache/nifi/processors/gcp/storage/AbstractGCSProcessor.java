...
public abstract class AbstractGCSProcessor extends AbstractGCPProcessor<Storage, StorageOptions> {
    ...
    protected StorageOptions getServiceOptions(ProcessContext context, GoogleCredentials credentials) {
        ...
        if (!StringUtils.isBlank(proxyHost) && proxyPort > 0) {
            ...
            HttpTransportOptions transportOptions = HttpTransportOptions.newBuilder().setHttpTransportFactory(() -> new NetHttpTransport.Builder()
                    .setProxy(new Proxy(Proxy.Type.HTTP, new InetSocketAddress(proxyHost, proxyPort))).build()).build();
